...
class Main(APIView):
    def get(self,request):
        ...
        return render(request, 'Hstagram/main.html', context=dict(feed_list=feed_list, user=user))
...
class Profile(APIView):
    def get(self,request):
        ...
        for feed in feed_object_list:
            like_count = FeedLike.objects.filter(feed_id=feed.id, is_like=True).count()
            is_like = FeedLike.objects.filter(feed_id=feed.id, is_like=True, email=email).exist()
